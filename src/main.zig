...
const Fluid = struct {
    ...
    pub fn fluid_step(self: *Fluid, iter: i32, N: i32) void {
        diffuse(1, self.Vx0, self.Vx, self.visc, self.dt, iter, N);
        diffuse(2, self.Vy0, self.Vy, self.visc, self.dt, iter, N);

        project(self.Vx0, self.Vy0, self.Vx, self.Vy, iter, N);

        advect(1, self.Vx, self.Vx0, self.Vx0, self.Vy0, self.dt, N);
        advect(2, self.Vy, self.Vy0, self.Vx0, self.Vy0, self.dt, N);

        project(self.Vx, self.Vy, self.Vx0, self.Vy0, iter, N);

        diffuse(0, self.s, self.density, self.diff, self.dt, iter, N);
        advect(0, self.density, self.s, self.Vx, self.Vy, self.dt, N);
    }
    ...
    fn render(self: *Fluid, scale: i32) void {
        ...
        for (0..size_usize) |j_usize| {
            ...
            for (0..size_usize) |i_usize| {
                ...
                const x = i * scale;
                const y = j * scale;
                const dens = self.density[IX(self.size, i, j)];
...
pub fn main() anyerror!void {
    ...
    while (!rl.windowShouldClose()) { // Detect window close button or ESC key
        ...
        fluid.mouseDragged(scale);
        fluid.fluid_step(iterations, N);
        fluid.clampDensity();
...
inline fn IX(N: i32, x: i32, y: i32) usize {
    ...
    const ix_usize = @as(usize, @intCast(ix_i));
    return ix_usize;
}
...
fn diffuse(b: i32, x: []f32, x0: []f32, diff: f32, dt: f32, iter: i32, N: i32) void {
    ...
    linear_solve(b, x, x0, a, 1 + 6 * a, iter, N);
}
    ...
    for (0..iter_usize) |_| {
        for (0..N_usize - 1) |j_usize| {
            ...
            for (0..N_usize - 1) |i_usize| {
                const i = @as(i32, @intCast(i_usize));
                x[IX(N, i, j)] = (x0[IX(N, i, j)] + a * (x[IX(N, i + 1, j)] + x[IX(N, i - 1, j)] + x[IX(N, i, j + 1)] + x[IX(N, i, j - 1)])) * cRecip;
            }
        }
        set_bnd(b, x, N);
    }
...
fn project(velocX: []f32, velocY: []f32, p: []f32, div: []f32, iter: i32, N: i32) void {
    ...
    set_bnd(0, div, N);
    set_bnd(0, p, N);
    linear_solve(0, p, div, 1, 6, iter, N);
...
fn advect(b: i32, d: []f32, d0: []f32, velocX: []f32, velocY: []f32, dt: f32, N: i32) void {
    ...
    for (0..N_usize - 1) |j_usize| {
        ...
        for (0..N_usize - 1) |k_usize| {
            ...
            tmp1 = dtx * velocX[IX(N, k, j)];
            tmp2 = dty * velocY[IX(N, k, j)];
...
fn set_bnd(b: i32, x: []f32, N: i32) void {
    ...
    for (0..N_usize - 1) |i_usize| {
        const i = @as(i32, @intCast(i_usize));
        x[IX(N, 0, i)] = if (b == 1) -x[IX(N, 1, i)] else x[IX(N, 1, i)];
        x[IX(N, N - 1, i)] = if (b == 1) -x[IX(N, N - 2, i)] else x[IX(N, N - 2, i)];
    }
